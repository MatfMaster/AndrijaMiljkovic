 ...
 * 
 * Great Britain
 * Checkout Test / Express shipping  / Guest / Dremel 3000
 * 
 */
public class CheckoutGuestUserExpressShipping_en_GB {
    ...
    @Test
    public void CheckoutGuestUserExpressShipping_en_GB() throws InterruptedException, IOException{
        try {
        ...
        if (!checkoutPage.page.getText().contains(ContentGb.ExpressShipping)) {
            Log.warn(ContentGb.ExpressShipping + " for Express SHIPPING not shown");
            landingPage.takeScreenshot(new Object(){}.getClass().getEnclosingMethod().getName());
            assertTrue(false);
        }
        ...
        Log.info("GB Checkout | Guest | Express shipping - Ended SUCCESSFULLY");
